...
resource "aws_iam_role" "aws_load_balancer_controller" {
  count              = var.eks_addons["aws-load-balancer-controller"] ? 1 : 0
  name               = "${var.name_prefix}irsa-eks-addon-load-balancer-controller-${local.oidc_id_substr}"
  assume_role_policy = local.aws_loadbalancer_controller_trust_policy_json
  tags               = merge({ Name = "${var.name_prefix}irsa-eks-addon-load-balancer-controller-${local.oidc_id_substr}" }, var.tags)
}
...
resource "aws_iam_policy" "aws_load_balancer_controller" {
  ...
  tags   = merge({ Name = "${var.name_prefix}AWSLoadBalancerControllerIAMPolicy" }, var.tags)
}
